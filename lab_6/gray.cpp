...
RunResult run_single_gray(int bits_per_dim, std::mt19937 &rng, int func_id, int dims) 
{
    ...
    for (int i = 0; i < POPULATION_SIZE; ++i) {
        ...
        if (f_calls_gray - 1 < MAX_F_CALLS * dims) {
    ...
    for (int gen = 0; gen < MAX_GENERATIONS && f_calls_gray < MAX_F_CALLS * dims; ++gen) {
        ...
        while (next_population.size() < (size_t)POPULATION_SIZE && f_calls_gray < MAX_F_CALLS * dims) {
            ...
            if (dis_prob(rng) < CROSSOVER_RATE) {
                int total_bits = dims * bits_per_dim;
                if (total_bits > 1) {
            ...
            auto evaluate_and_add = [&](Individual_gray& ind) -> bool {
                if (f_calls_gray >= MAX_F_CALLS * dims) return false; // defensive
                ...
                if (f_calls_gray - 1 < MAX_F_CALLS * dims) {
                    ...
                    // if we've exceeded MAX_F_CALLS * dims, do not add to population
                    return false;
                }
    ...
    for (int i = f_calls_gray; i < MAX_F_CALLS * dims; ++i) {
